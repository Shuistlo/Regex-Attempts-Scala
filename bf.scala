...
object CW8b {
  ...
  def jumpRight(prog: String, pc: Int, level: Int): Int = {
    val start = prog.indexOf("[", pc)
    val end = prog.indexOf("]", pc)
    if(end == -1) prog.length()
  ...
  def jumpLeft(prog: String, pc: Int, level: Int): Int = {
    val start = prog.lastIndexOf("]", pc)
    val end = prog.lastIndexOf("[", pc)
    if (end == -1) -1
    else if (level == 0 && (end > start || start == -1)) end + 1
    else if (end > start || start == -1) jumpLeft(prog, end - 1, level + 1)
    else jumpLeft(prog, start - 1, level - 1)
  }
  ...
  def run(prog: String, pc: Int, mp: Int, mem: Mem): Mem = {
    if (pc >= prog.length()) mem
      ...
      case '-' => run(prog, pc + 1, mp, write(mem, mp, sread(mem, mp) - 1))
      case '.' => print(sread(mem, mp).toChar); run(prog, pc + 1, mp, mem) //needs to print as a character
      case ',' => run(prog, pc + 1, mp, mem + (mp -> Console.in.read().toByte))
      case '[' => if (sread(mem, mp) == 0) run(prog, jumpRight(prog, pc + 1, 0), mp, mem) else run(prog, pc + 1, mp, mem)
      case ']' => if (sread(mem, mp) != 0) run(prog, jumpLeft(prog, pc - 1, 0), mp, mem) else run(prog, pc + 1, mp, mem)
      case anythingElse => run(prog, pc + 1, mp, mem)
    }
  }

  def start(prog: String, mem: Mem) = run(prog, 0, 0, mem)
